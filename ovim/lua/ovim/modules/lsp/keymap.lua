...
function K.lsp()
  local m = {
    ...
    ["n|]d"] = map_cr("Lspsaga diagnostic_jump_next", opts),
    ["n|[d"] = map_cr("Lspsaga diagnostic_jump_prev", opts),
    ...
    ["n|<leader>gD"] = map(vim.lsp.buf.definition, opts):display("Definition"),
    ["n|<leader>gt"] = map(vim.lsp.buf.type_definition, opts):display("TypeDefinition"),
    ["n|<leader>gi"] = map(vim.lsp.buf.implementation, opts):display("Implementation"),
    ["n|<leader>gn"] = map_cr("Lspsaga rename", opts):display("Rename"),
    ["n|<leader>gc"] = map(vim.lsp.buf.declaration, opts):display("Declaration"),
    ["n|<leader>gu"] = map(vim.lsp.buf.references, opts):display("References"),
    ["n|<leader>gf"] = map(function()
      vim.lsp.buf.format { async = true }
    end, opts):display("Format"),
